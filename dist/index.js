...
function create(queryable, collection) {
    ...
    if (_map.length) {
      result = _map.reduce(function (result, mapper) {
        return result.map(function (item, index) {
          return typeof mapper === "function" ? mapper(item, index) : item[mapper]();
        });
      }, result);
    }
  ...
  var query = (_query = {}, _defineProperty(_query, copy, function (data) {
    ...
    var currentUnsubscribes = buildQueries().map(function (queryable) {
      return queryable.onSnapshot(options, callback);
    });
    unsubscribes.push.apply(unsubscribes, _toConsumableArray(currentUnsubscribes));
    return function () {
      currentUnsubscribes.forEach(function (unsubscribe) {
        unsubscribe();
        var index = unsubscribes.indexOf(unsubscribe);
        if (index !== -1) {
